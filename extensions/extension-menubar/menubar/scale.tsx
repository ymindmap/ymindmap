...
export const ScaleMenubar = (props: MenubarProps) => {
    ...
    const getZoom = function() {
        const zoom: number = Array.isArray(app.tree.scale) ? app.tree.scale[0] : app.tree.scale;
        return zoom || 1;
    }
    ...
    const update = function() {
        ...
        (zoomInButton as unknown as HTMLButtonElement).disabled = zoom <= props.options.min;

        if (zoom > props.options.max) zoom = props.options.max;
        (zoomOutButton as unknown as HTMLButtonElement).disabled = zoom >= props.options.max;
        (zoomValue as unknown as HTMLDivElement).innerHTML = `${(zoom * 100).toFixed(0)}%`
        if (zoom !== currentZoom) childButtonProps.zoomTo(zoom);
    }
    props.board.view.app.on(ZoomEvent.ZOOM, update);
    update();

    return <div style={{ left: 0, bottom: 0, display: 'flex', 'flexDirection': 'row' }}>
            {zoomInButton}
            {zoomValue}
            {zoomOutButton}
           </div>;
